      ...
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex md:hidden items-center gap-0.5">
            {navLinks.map(link => {
              const isActive = link.href === '/ranking/sword'
                ? pathname.startsWith('/ranking/')
                : pathname === link.href
              return (
                <Link
                  ...
                  href={link.href}
                  className={cn(
                    'px-2 py-1.5 text-[11px] font-medium rounded-lg transition-colors',
                    isActive ? 'text-amber-400 bg-amber-500/10' : 'text-muted-foreground'
                  )}
                >
                  {link.label}
                </Link>
